  ...
  return (
    <div className="sticky top-0 z-50 w-full px-4 pt-2">
      <header className="glass mx-auto max-w-5xl rounded-2xl border border-border/50">
        <div className="flex h-14 items-center justify-between px-5">
          <div className="flex items-center gap-5">
            <Link href="/" className="flex items-center gap-2 group">
              ...
              <span className="hidden sm:inline text-lg font-bold text-foreground transition-colors duration-300 group-hover:text-primary">LuWatch</span>
            </Link>
            ...
            <nav className="hidden md:flex items-center gap-0.5">
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`relative px-3 py-1.5 text-sm font-medium transition-all duration-200 rounded-lg hover:bg-primary/10 ${
                    pathname === link.href
                      ? 'text-primary'
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t border-border/50 animate-fade-in-down">
